...
export async function generateDailyLesson(request, operation = 'generate') {
  ...
  try {
    ...
    const recent = existing.some(r => r.fields.Status === 'Ready' || r.fields.Status === 'Processing' || (r.fields.Status === 'Posted' && Date.parse(r.fields['Posted At']) > Date.now() - 13 * 86400000));
    if (recent) {
