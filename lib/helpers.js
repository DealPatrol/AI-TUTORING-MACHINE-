...
export async function listReadyQueue(type) {
  try {
    ...
    if (type === "Reel") {
      const withVideo = ready.filter((r) => extractQueueVideo(r.fields));
    ...
    // Feed / default: only take image posts without a video so reel cron can take videos
    const feedLike = ready.filter((r) => r.fields["Image URL"] && !extractQueueVideo(r.fields));
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        const withoutVideo = ready.filter((r) => !extractQueueVideo(r.fields));
      ...
      const withImage = ready.filter(
        (r) => r.fields["Image URL"] && !extractQueueVideo(r.fields)
      );
