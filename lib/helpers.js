...
export async function listReadyQueue(type) {
  try {
    ...
    // Feed / default: prefer image posts without a video URL so reel cron can take videos
    const feedLike = ready.filter((r) => r.fields["Image URL"] && !r.fields["Video URL"]);
    if (feedLike.length) return [feedLike[0]];
    const anyImage = ready.filter((r) => r.fields["Image URL"]);
    return anyImage.length ? [anyImage[0]] : [];
  }
}
...
export async function listReadyBySequence(sequence) {
  try {
    ...
    if (
      ...
      const ready = await listReadyByStatusOnly();
      const withImage = ready.filter((r) => r.fields["Image URL"]);
