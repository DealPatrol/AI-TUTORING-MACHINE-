...
// Gemini text rewrite with 429 retries — used when Claude credits are exhausted.
export async function geminiRewrite(prompt, { retries = 3 } = {}) {
  "gemini-3.6-flash",
  "gemini-3.5-flash",
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const imgRes = await fetch(
        `${GEMINI_BASE}/models/gemini-2.5-flash-image:generateContent?key=${getGeminiApiKey()}`,
        `${GEMINI_BASE}/models/gemini-3.1-flash-image:generateContent?key=${getGeminiApiKey()}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
...
async function listReadyByStatusOnly() {
  return airtableList(
    ...
    { paginate: true }
    "maxRecords=5&filterByFormula=" + encodeURIComponent(`{Status}="Ready"`)
  );
}
