...
export async function airtableCreateQueue(fields) {
  ...
  try {
    ...
    if (fields.Type && fields.Type !== "Feed") {
      throw new Error(
        `Airtable Queue is missing growth fields required for ${fields.Type}. Add: Type, Video URL, Cover URL, First Comment, Slide URLs, Story Text, Story Image URL. See AIRTABLE_SETUP.md`
      );
    }
    const stripped = { ...fields };
    for (const key of optionalForFeed) delete stripped[key];
    console.warn(
      "Queue missing optional growth fields — created Feed with core fields only. Add Type/Video URL/Cover URL/First Comment/Slide URLs/Story fields in Airtable."
    );
