// SKILL 04b — THE POSTER #2 (runs daily at 2 PM UTC)
// Publishes the second post (Sequence=2) from the Queue to Instagram.
...
export async function GET(request) {
  ...
  try {
    // 1. Find the second Ready post (Sequence=2)
    const queue = await airtableList(
      "Queue",
      "maxRecords=1&filterByFormula=" + encodeURIComponent(`AND({Status}="Ready", {Sequence}=2)`)
    );
    if (queue.length === 0) {
      return Response.json({ ok: true, message: "No Sequence=2 posts ready" });
    }
    const post = queue[0];
    ...
    return Response.json({ ok: true, posted: post.fields.Hook, sequence: 2, igMediaId: published.id });
  } catch (err) {
    console.error("Post-2 cron error:", err);
